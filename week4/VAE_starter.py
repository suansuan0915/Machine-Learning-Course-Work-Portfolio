...
def Plot_Kernel(_model):
    ...
    print(type(weights))
    print(weights.size())
    reshaped_kernel = weights.view(-1,28,28)
    kernels = reshaped_kernel.cpu().detach() 
    kernel_np = kernels.numpy()
    
    fig = plt.figure(figsize=(15,15))
    plt.title(f'Plot of kernel for Each Model (Weights of the First Layer)')
    for i in range(10):
        fig.add_subplot(5, 2, i+1)
        plt.imshow(kernel_np[i])
    plt.show()
